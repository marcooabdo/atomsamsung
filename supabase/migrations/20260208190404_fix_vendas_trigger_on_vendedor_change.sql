/*
  # Corrigir trigger de vendas para reagir a mudanças no vendedor

  1. Problema
    - Trigger atual só dispara em INSERT ou UPDATE OF status
    - Quando o vendedor_id é alterado, não recalcula estrelas
    - Estrelas ficam no vendedor antigo

  2. Solução
    - Modificar trigger para disparar também em UPDATE OF vendedor_id
    - Quando vendedor mudar, recalcular estrelas para AMBOS (antigo e novo)
    - Garantir que vendas canceladas removam estrelas corretamente
*/

-- Recriar função com suporte a mudança de vendedor
...
BEGIN
  -- Se está mudando o vendedor, precisamos recalcular para o vendedor ANTIGO também
  IF TG_OP = 'UPDATE' AND OLD.vendedor_id IS DISTINCT FROM NEW.vendedor_id AND OLD.status = 'concluido' THEN
    -- Buscar profissional do vendedor ANTIGO
    ...
    IF v_old_profissional_id IS NOT NULL THEN
      ...
      IF v_pilar_nome IS NOT NULL THEN
        ...
        IF v_pilar_id IS NOT NULL THEN
          -- Contar vendas do vendedor ANTIGO neste mês
          ...
          WHERE vendedor_id = OLD.vendedor_id
            ...
            AND id != NEW.id; -- Excluir a venda atual que está sendo modificada
          ...
          -- Atualizar estrelas do vendedor ANTIGO
          ...
          ELSE
            -- Se não tem mais vendas, remover registro
  ...
  -- Processar venda concluída ou mudança de status
  IF NEW.status = 'concluido' THEN
    -- Buscar profissional do vendedor NOVO
    ...
    -- Contar total de vendas CONCLUÍDAS do vendedor NOVO neste mês e tipo
    SELECT COUNT(*) INTO v_total_vendas_mes
    FROM vendas
  ...
  ELSIF NEW.status = 'cancelado' AND (TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.status != 'cancelado')) THEN
    ...
    IF v_profissional_id IS NOT NULL THEN
      ...
      IF v_pilar_nome IS NOT NULL THEN
        ...
        IF v_pilar_id IS NOT NULL THEN
          -- Contar vendas concluídas restantes
          SELECT COUNT(*) INTO v_total_vendas_mes
          FROM vendas
          WHERE vendedor_id = NEW.vendedor_id
            AND tipo_venda = NEW.tipo_venda
          ...
          IF v_total_vendas_mes > 0 THEN
            ...
            WHERE profissional_id = v_profissional_id
              ...
              AND pilar_id = v_pilar_id;
          ELSE
            -- Remover registro se não tem mais vendas
            DELETE FROM skywalker_estrelas_mes
            WHERE profissional_id = v_profissional_id
              AND mes_referencia = v_mes_ref
              AND pilar_id = v_pilar_id;
...
-- Recriar o trigger para disparar em mudanças de vendedor_id também
DROP TRIGGER IF EXISTS trigger_registrar_venda_skywalker ON vendas;

CREATE TRIGGER trigger_registrar_venda_skywalker
  BEFORE INSERT OR UPDATE OF status, vendedor_id ON vendas
  FOR EACH ROW
  EXECUTE FUNCTION registrar_venda_skywalker();

-- Comentário
COMMENT ON FUNCTION registrar_venda_skywalker() IS 'Calcula estrelas Skywalker baseado no total acumulado de vendas do mês. Recalcula quando vendedor ou status mudam.';
