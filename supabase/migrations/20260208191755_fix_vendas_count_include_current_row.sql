/*
  # Corrigir contagem de vendas para incluir a linha atual

  1. Problema
    - Trigger usa COUNT(*) que não inclui a linha sendo modificada
    - Quando edita Store+ → Care+, conta 0 vendas Care+ ao invés de 1

  2. Solução
    - Usar uma subquery que inclui a linha atual (NEW)
    - Contar vendas do mesmo vendedor, tipo e mês
*/
...
BEGIN
    ...
    IF v_profissional_id IS NOT NULL THEN
      ...
      IF v_old_pilar_nome IS NOT NULL THEN
        ...
        IF v_old_pilar_id IS NOT NULL THEN
          -- Contar vendas do tipo ANTIGO (excluindo esta linha que está mudando)
  ...
  IF NEW.status = 'concluido' THEN
    ...
    -- CORRIGIDO: Contar vendas do NOVO tipo incluindo outras + esta sendo modificada
    -- Conta vendas JÁ no banco do mesmo tipo E inclui esta linha
    SELECT COUNT(*) + 1 INTO v_total_vendas_mes
    FROM vendas
    WHERE vendedor_id = NEW.vendedor_id
      AND tipo_venda = NEW.tipo_venda
      AND status = 'concluido'
      AND date_trunc('month', created_at)::date = v_mes_ref
      AND id != NEW.id;  -- Exclui a linha atual e soma +1 no final
...
COMMENT ON FUNCTION registrar_venda_skywalker() IS 'Calcula estrelas Skywalker baseado no total acumulado. Inclui a linha atual na contagem.';
