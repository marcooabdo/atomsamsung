/*
  # Corrigir lógica de contagem definitivamente

  1. Problema
    - COUNT não inclui a linha sendo modificada (NEW)
    
  2. Solução Correta
    - Para INSERT: COUNT + 1 (a nova venda)
    - Para UPDATE concluído: COUNT de TODAS incluindo esta
*/
...
BEGIN
    ...
    IF v_profissional_id IS NOT NULL THEN
      ...
      IF v_old_pilar_nome IS NOT NULL THEN
        ...
        IF v_old_pilar_id IS NOT NULL THEN
          -- Contar vendas do tipo ANTIGO (excluindo esta que está mudando)
  ...
  IF NEW.status = 'concluido' THEN
    ...
    -- CORRIGIDO: Contar TODAS as vendas do mesmo tipo (incluindo esta se for UPDATE)
    IF TG_OP = 'INSERT' THEN
      -- Para INSERT: conta as existentes + 1 (a nova)
      ...
      WHERE vendedor_id = NEW.vendedor_id
        ...
        AND date_trunc('month', created_at)::date = v_mes_ref;
    ELSE
      -- Para UPDATE: conta TODAS (incluindo esta que já existe no banco)
      SELECT COUNT(*) INTO v_total_vendas_mes
      FROM vendas
      WHERE vendedor_id = NEW.vendedor_id
        AND tipo_venda = NEW.tipo_venda
...
COMMENT ON FUNCTION registrar_venda_skywalker() IS 'Calcula estrelas Skywalker. INSERT conta +1, UPDATE conta todas do tipo.';
