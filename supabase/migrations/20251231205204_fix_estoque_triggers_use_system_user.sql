/*
  # Fix Estoque Triggers - Use System User
  
  1. Changes
    - Update log_estoque_pecas_movement to use system user as fallback
    - Update log_requisicoes_pecas_changes to use system user as fallback
    - Ensure all user names are properly shown in history
  
  2. Security
    - Maintains RLS policies
    - System user used only as fallback
*/

-- Update log_estoque_pecas_movement
...
DECLARE
  ...
  v_os_numero text;
BEGIN
  v_usuario_id := COALESCE(auth.uid(), NEW.tecnico_id, OLD.tecnico_id, '00000000-0000-0000-0000-000000000001'::uuid);
...
-- Update log_requisicoes_pecas_changes
...
DECLARE
  ...
  v_usuario_id uuid;
BEGIN
  v_usuario_id := COALESCE(auth.uid(), NEW.requisitado_por, OLD.requisitado_por, '00000000-0000-0000-0000-000000000001'::uuid);
  SELECT nome INTO v_usuario_nome FROM usuarios WHERE id = v_usuario_id;
