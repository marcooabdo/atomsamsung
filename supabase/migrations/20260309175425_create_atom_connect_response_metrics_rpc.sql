/*
  # Create Atom Connect Response Time Metrics RPC

  1. New Functions
    - `get_atom_connect_response_metrics(p_unidade_id uuid)`
      - Returns response time metrics for the Atom Connect dashboard
      - Calculates first response time (time from first client message to first operator response)
      - Calculates average time between client message and operator response
      - Breaks down metrics per attendant
      - Identifies oldest contact waiting for response
      - Counts conversations with expired SLA (20 min threshold)

  2. Notes
    - Uses atom_connect_mensagens table to compute all timing metrics
    - from_me = false means client message, from_me = true means operator/bot response
    - Only considers non-bot responses (is_bot = false) for operator metrics
    - SLA threshold is 20 minutes for first response
*/
