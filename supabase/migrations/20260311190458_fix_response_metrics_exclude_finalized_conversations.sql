/*
  # Fix Response Metrics RPC - Exclude Finalized Conversations

  1. Changes
    - Updated `get_atom_connect_response_metrics` RPC function
    - Added filter `resultado_conversa IS NULL` to the base CTE (conversa_first_messages)
      so that finalized conversations are excluded from:
      - SLA expired count
      - Oldest waiting contact
      - All waiting contacts list
      - Average first response time
      - Average between-response time
      - Per-attendant metrics

  2. Notes
    - Conversations with `resultado_conversa IS NOT NULL` are considered finalized
    - These should NOT count toward active SLA metrics or "aguardando resposta"
    - Only active (non-finalized) conversations are now included in all calculations
*/
