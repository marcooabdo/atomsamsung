/*
  # Add UPDATE Trigger for Payment OS Linking

  1. Problem
    - The trigger `atualizar_valores_os()` only fires on INSERT
    - When payments are linked to OS via UPDATE (cotacao_id -> os_id), trigger doesn't fire
    - This causes saldo_restante to be incorrect when cotacao is approved

  2. Solution
    - Add UPDATE trigger to recalculate OS values when payments are linked
    - Modify function to handle both OLD and NEW records
    - Recalculate for both old and new OS if os_id changes

  3. Changes
    - Drop and recreate function to handle UPDATE scenario
    - Add UPDATE and DELETE triggers
    - Handle os_id changes properly
*/
...
-- Recreate function with support for UPDATE (OLD and NEW)
