...
export function MapaEstoque() {
  ...
  useEffect(() => {
    if (selectedUnidade || usuario?.unidade_id) {
      loadSalas();
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Mapa do Estoque</h1>
            <p className="text-sm text-gray-400">Localização visual e gestão de layout físico</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <UnitFilter onUnitChange={setSelectedUnidade} />
      ...
      <div className="premium-card p-6">
        ...
        {salas.length === 0 ? (
          ...
          <ConfigView
            ...
            selectedUnidade={selectedUnidade || usuario?.unidade_id || ''}
          />
        ) : (
          <MapView
            salas={salas}
...
function ConfigView({
  ...
  const [newEstante, setNewEstante] = useState({ nome: '', andares: 4, bins_por_andar: 6 });
  ...
  const handleCreateEstante = async () => {
    ...
    try {
      ...
      for (let andar = 1; andar <= newEstante.andares; andar++) {
        for (let posicao = 1; posicao <= newEstante.bins_por_andar; posicao++) {
          binsToCreate.push({
            estante_id: estanteData.id,
            andar,
            posicao,
            codigo: `${newEstante.nome.substring(0, 3).toUpperCase()}-${andar}-${posicao}`
          });
        }
      }
      ...
      setNewEstante({ nome: '', andares: 4, bins_por_andar: 6 });
          ...
          <div className="premium-card w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Andares</label>
                  <input
                    type="number"
                    min="1"
                    max="20"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Bins por Andar</label>
                  <input
                    type="number"
                    min="1"
                    max="50"
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {estante.andares} andares × {estante.bins_por_andar} bins = {estante.andares * estante.bins_por_andar} total
                      </p>
                    </div>
                  </div>
                </div>
