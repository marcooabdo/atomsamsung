    ...
    <div className="fixed inset-0 z-50 flex justify-end" style={{ background: 'rgba(10,10,13,0.7)', backdropFilter: 'blur(4px)' }} onClick={onClose}>
      <div className="relative w-full max-w-xl h-full overflow-hidden flex flex-col"
        ...
        style={{
          ...
          boxShadow: '-10px 0 40px rgba(0,0,0,0.5), 0 0 60px rgba(var(--accent-rgb),0.1)',
          animation: 'slideInRight 0.3s ease-out',
        }}>

        {showConfetti && <Confetti />}

        <div className="flex items-start justify-between px-6 py-5 border-b relative" style={{ borderColor: 'var(--border-primary)' }}>
          <div className="flex-1 min-w-0 pr-4">
            ...
            <h2 className="text-lg font-bold truncate" style={{ color: 'var(--text-primary)' }}>{ocorrencia.titulo}</h2>
        ...
        <div className="flex-1 overflow-y-auto cyber-scrollbar p-6 space-y-5">
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-primary)' }}>
              <div className="text-[10px] uppercase tracking-wider font-bold mb-1" style={{ color: 'var(--text-secondary)' }}>Total</div>
              <div className="text-sm font-mono font-bold" style={{ color: 'var(--text-primary)' }}>{formatBRL(ocorrencia.valor_total)}</div>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] uppercase tracking-wider font-bold" style={{ color: 'var(--text-secondary)' }}>Progresso</span>
              <span className="text-xs font-mono font-bold" style={{ color: 'var(--text-accent)' }}>{ocorrencia.percentual_pago.toFixed(1)}%</span>
            </div>
            ...
            {ocorrencia.responsaveis.map(resp => {
              ...
              return (
                <div key={resp.id} className="rounded-xl overflow-hidden"
                  style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-primary)' }}>
                  <div className="p-4 border-b" style={{ borderColor: 'var(--border-primary)' }}>
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-10 h-10 rounded-full overflow-hidden flex items-center justify-center flex-shrink-0"
                        ...
                        {resp.usuario_foto ? <img src={resp.usuario_foto} alt="" className="w-full h-full object-cover" /> : <User className="w-5 h-5" style={{ color: 'var(--text-accent)' }} />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-bold truncate" style={{ color: 'var(--text-primary)' }}>{resp.usuario_nome}</div>
                        <div className="text-[10px]" style={{ color: 'var(--text-secondary)' }}>{resp.percentual.toFixed(2)}% · {formatBRL(resp.valor_devido)}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--text-secondary)' }}>Pago</div>
                      ...
                      <label key={p.id} className="flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition hover:bg-white/5">
                        <button type="button" onClick={() => toggleParcela(p)}
                          ...
                          style={{
                            ...
                            border: `1.5px solid ${p.deduzido ? '#4ADE80' : 'var(--border-primary)'}`,
                        ...
                        <div className="flex-1 min-w-0 flex items-center gap-3">
                          <div className="text-xs font-mono font-bold w-12" style={{ color: 'var(--text-accent)' }}>{p.numero_parcela}/{p.total_parcelas}</div>
                          <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-secondary)' }}>
                            <Calendar className="w-3 h-3" />
                            {new Date(p.mes_referencia + 'T00:00').toLocaleDateString('pt-BR', { month: 'short', year: 'numeric' })}
                          </div>
                        </div>
                        <div className="text-sm font-mono font-bold" style={{ color: p.deduzido ? '#4ADE80' : 'var(--text-primary)' }}>
                          {formatBRL(p.valor)}
                        </div>
                      </label>
                    ))}
          ...
          {ocorrencia.status === 'quitado' && (
            <div className="p-4 rounded-xl flex items-center gap-3"
              ...
              <div>
                <div className="text-sm font-bold" style={{ color: '#4ADE80' }}>Ocorrencia Quitada</div>
                <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>Todos os valores foram deduzidos</div>
              </div>
            </div>
          )}
        </div>
