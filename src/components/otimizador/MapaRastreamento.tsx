...
import { MapPin, Users, Eye, RefreshCw, Filter, Navigation } from 'lucide-react';
...
export default function MapaRastreamento() {
  ...
  if (!isLoaded) {
    return <div className="flex items-center justify-center h-96"><div className="w-8 h-8 border-4 rounded-full animate-spin" style={{ borderColor: 'var(--text-accent)', borderTopColor: 'transparent' }} /></div>;
  }
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            {tecnicos.map(t => <option key={t.tecnico_id} value={t.tecnico_id}>{t.tecnico_nome}</option>)}
          </select>

          <button
            onClick={() => setShowTecnicos(!showTecnicos)}
            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium ${showTecnicos ? 'ring-2' : ''}`}
            style={{ backgroundColor: showTecnicos ? '#10B98120' : 'var(--bg-secondary)', color: showTecnicos ? '#10B981' : 'var(--text-secondary)', ringColor: '#10B981' }}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium ${showAgenda ? 'ring-2' : ''}`}
            style={{ backgroundColor: showAgenda ? '#3B82F620' : 'var(--bg-secondary)', color: showAgenda ? '#3B82F6' : 'var(--text-secondary)', ringColor: '#3B82F6' }}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium ${autoRefresh ? 'ring-2' : ''}`}
            style={{ backgroundColor: autoRefresh ? '#F59E0B20' : 'var(--bg-secondary)', color: autoRefresh ? '#F59E0B' : 'var(--text-secondary)', ringColor: '#F59E0B' }}
          >
            <RefreshCw className={`w-3.5 h-3.5 ${autoRefresh ? 'animate-spin' : ''}`} style={{ animationDuration: '3s' }} />
            Auto
          </button>

          <button onClick={loadData} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ backgroundColor: 'var(--text-accent)', color: 'var(--text-on-accent)' }}>
      ...
      <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border-primary)' }}>
        ...
        >
          {baseLoc && (
            <Marker
              position={baseLoc}
              icon={{ url: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="#10B981" stroke="white" stroke-width="2"><rect x="4" y="4" width="16" height="16" rx="2"/><path d="M9 9h6v6H9z"/></svg>'), scaledSize: new google.maps.Size(36, 36), anchor: new google.maps.Point(18, 18) }}
              title="Base - Unidade"
          ...
          {showTecnicos && filteredTecnicos.map(t => (
            <Marker
              ...
              icon={{ url: 'data:image/svg+xml,' + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40"><circle cx="20" cy="20" r="18" fill="${t.em_atendimento ? '#F59E0B' : t.presence_status === 'online' ? '#10B981' : '#6B7280'}" stroke="white" stroke-width="3"/><text x="20" y="25" text-anchor="middle" fill="white" font-size="14" font-weight="bold">${(t.tecnico_nome || '?')[0]}</text></svg>`), scaledSize: new google.maps.Size(40, 40), anchor: new google.maps.Point(20, 20) }}
              onClick={() => setSelectedMarker({ type: 'tecnico', data: t })}
            />
          ))}

          {showAgenda && filteredAgenda.map((a, idx) => (
            <Marker
              key={`ag-${a.id}`}
              position={{ lat: a.lat, lng: a.lng }}
              icon={{ url: 'data:image/svg+xml,' + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="40" viewBox="0 0 32 40"><path d="M16 0C7.2 0 0 7.2 0 16c0 12 16 24 16 24s16-12 16-24C32 7.2 24.8 0 16 0z" fill="${statusColor(a.status)}"/><text x="16" y="20" text-anchor="middle" fill="white" font-size="12" font-weight="bold">${idx + 1}</text></svg>`), scaledSize: new google.maps.Size(28, 35), anchor: new google.maps.Point(14, 35) }}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-xl p-4" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-primary)' }}>
          <h3 className="font-bold text-sm mb-3 flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
            ...
            Tecnicos ({filteredTecnicos.length})
          </h3>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {filteredAgenda.map((a, i) => (
              <div key={a.id} className="flex items-center justify-between p-2 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                <div className="flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold text-white" style={{ backgroundColor: statusColor(a.status) }}>{i + 1}</span>
                  <div>
                    <span className="text-xs font-medium" style={{ color: 'var(--text-primary)' }}>OS {a.numero_os}</span>
                    <span className="text-xs ml-2" style={{ color: 'var(--text-secondary)' }}>{a.cliente_cidade}</span>
                  </div>
                </div>
                <span className="text-xs" style={{ color: statusColor(a.status) }}>
                  {a.status === 'concluido' ? 'OK' : a.status === 'em_atendimento' ? 'Atendendo' : a.periodo || 'Pendente'}
                </span>
              </div>
            ))}
            {filteredAgenda.length === 0 && <p className="text-xs text-center py-4" style={{ color: 'var(--text-secondary)' }}>Nenhum atendimento agendado</p>}
          </div>
        </div>
      </div>
    </div>
