...
export default function MotorOtimizacaoNew() {
  ...
  const runOptimization = useCallback(() => {
    ...
    const osComCoord = filteredOS.filter(os => os.lat && os.lng);
    const osSemCoord = filteredOS.filter(os => !os.lat || !os.lng);
          ...
          >
            {baseCoords && (
              <Marker
                position={baseCoords}
                icon={{
                  url: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40"><circle cx="20" cy="20" r="18" fill="#10B981" stroke="white" stroke-width="3"/><text x="20" y="26" text-anchor="middle" fill="white" font-size="18" font-weight="bold">A</text></svg>'),
            ...
            {paradas.map((p, idx) => (
              <Marker
                ...
                icon={{
                  url: 'data:image/svg+xml,' + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="34" height="42" viewBox="0 0 34 42"><path d="M17 0C7.6 0 0 7.6 0 17c0 12.8 17 25 17 25s17-12.2 17-25C34 7.6 26.4 0 17 0z" fill="${p.os.rota_cor || '#3B82F6'}"/><circle cx="17" cy="17" r="12" fill="white"/><text x="17" y="22" text-anchor="middle" fill="${p.os.rota_cor || '#3B82F6'}" font-size="12" font-weight="bold">${idx + 1}</text></svg>`),
